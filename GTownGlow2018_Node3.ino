//Node 3 is the smaller figure - a child in a tree waving both arms up and down in sync (hence no need for left arm)
...
#define PIN3 34   // left arm - static --> !!! NOT USING FOR NODE 3 !!!
#define PIN4 44   // right leg
#define PIN5 46   // left leg

#define HeadLEDs 82           //82
#define BodyLEDs 39           //39
#define RightArmLEDs 74       //74
#define RightArmLEDsUp 74     //74
#define RightArmLEDsDown 74   //74
//  #define LeftArmLEDs 74    // --> !!! NOT USING FOR NODE 3 !!!
#define RightLegLEDs 36       //36
#define leftLegLEDs 36        //36

const int numberOfStrips = 7;
...
// NeoPatterns Strand2(LeftArmLEDs, PIN3, NEO_GRB + NEO_KHZ800, &Strand2Complete); // --> !!! NOT USING FOR NODE 3 !!!
...
const int lightSensorThreshold = 75;
...
void loop()
{
  ...
  loopCount++;
  if(loopCount >= 10,000) { loopCount = 1; }

  Serial.print("Light value: ");
  Serial.println(sensorValue);

  // RECEIVING from Node 2
...
if (sensorValue < lightSensorThreshold)
{
  ...
  else   // no message received; do your own thing
  {
    // CHOOSE RANDOM COLOR !!!
      ...
      for (int i = 0; i < numberOfStrips; i++)
        {
          if(i != 1 && i != 3)  // exclude waving arms
          {
            // draw main arm from center
            if(i == 2)
            {
              drawNeopixelLineFromCenter(i, currentColor, 35);
            }
            else
            {
              drawNeoPixelLine(i, currentColor);
        ...
        // !!!  Wave every 12 times through the loop
        if(loopCount % 12 == 0)
        {
          wave(currentColor);
        }
...
}  // sensorValue < lightSensorThreshold 
...
void drawNeopixelLineFromCenter(int stripNum, uint32_t lineColor, int delayAmount)
{
  // this code draws one line/strand starting at middle and going outward in both directions
  // 
  //uint32_t lineColor = strip[stripNum].Color(85, 191, 63);

  int centerPix = strip[stripNum].numPixels() / 2;
  int i = centerPix;

  for (int j=centerPix; j <= strip[stripNum].numPixels(); j++) 
  {
    strip[stripNum].setPixelColor(j, lineColor);
    strip[stripNum].setPixelColor(i, lineColor);
    strip[stripNum].show();
    i--;
...
void wave(uint32_t waveColor)
{
  ...
  {
      // assume starting with upper arm lit
      ledsOff(2, 1);
      drawNeopixelLineFromCenter(1, waveColor, 0);
      delay(delayVal);

      ledsOff(1, 1);
      drawNeopixelLineFromCenter(3, waveColor, 0);
      delay(delayVal);
      ...
      ledsOff(1, 1);
      drawNeopixelLineFromCenter(2, waveColor, 0);
      delay(delayVal);
  }
  

/*
  drawNeopixelLineFromCenter(1, strip[1].Color(random(200), random(0), random(0)), 5);
  delay(200);
  drawNeopixelLineFromCenter(2, strip[2].Color(random(0), random(200), random(0)), 5);
  delay(200);
  drawNeopixelLineFromCenter(3, strip[3].Color(random(0), random(0), random(200)),5);
  delay(200);
*/
...
void waveXXX(uint32_t waveColor)
{
  uint32_t offColor = strip[1].Color(random(0), random(0), random(0));
  int startPixel = 0;
  ...
  for(int n=0; n < 5; n++)  // how many times to wave
  {
    ...
    //wave down
    // don't want to repeat bottom arm (strip[1]) repeating
    //  hence i > 1 condition
    for(int i=3; i > 1; i--)  // right arm is [1, 2, 3]
    {
      if(i == 2)
        { startPixel = strip[2].numPixels() / 2;}
      else
